...
- **같은 target-version에 대한 분석 중복 실행을 막는 장치가 없음**: `run-analysis`/`rerun`을 짧은 시간에 두 번 호출하면(더블클릭, 여러 명이 동시에 같은 언어판을 재분석 등) 둘 다 기존 결과를 지우고 각자 백그라운드 태스크를 띄웁니다 — API 비용이 중복 소모되고 먼저 끝난 쪽 결과가 나중 것에 덮어써질 수 있습니다. 같은 이유로 같은 finding을 두 검수자가 동시에 수정하면 나중 요청이 그냥 덮어씁니다(락/버전 체크 없음, last-write-wins). 로그인 계정 구분 없이 사이트 전체가 Caddy Basic Auth 하나로만 보호되므로(배포 섹션 참고), 여러 명이 같은 작품을 동시에 검수하는 워크플로우에는 안전하지 않습니다 — 작품/에피소드 단위로 담당자를 나눠 쓰는 걸 전제합니다.
