...
_GENDER_RESOLUTION_SYSTEM_PREFIX = (
    ...
    "후보의 성별과 무관하니 근거로 쓰지 마라(예: \"저 선생님 사랑하고 "
    "있어요\"에서 \"선생님\"은 청자를 가리킬 뿐, 화자 자신의 성별과는 "
    "무관하다 — 화자를 가리키는 단어는 \"저\"뿐이고 이건 성별 무관 "
    "대명사이므로 이 문장은 화자 성별에 대해 null이어야 한다). "
    "A) 판정 대상 인물 자신을 직접 가리키는 단어가 있으면(호칭, 이름, "
    "\"여자/남자/아들/딸\" 같은 지칭어, \"새끼/인간\" 같은 속어적 지칭 "
    "표현, 이미 성별이 확정된 다른 후보와의 명확한 동일 인물 관계 등) — "
    "100% 확신이 아니어도 좋으니 그 단서로 가장 합리적인 \"male\" 또는 "
    "\"female\"을 채워라. "
    "B) 판정 대상 인물 자신을 가리키는 단어가 없고 \"나\", \"너\", "
    "\"저\", \"걔\", \"-요/-네\" 같은 성별 무관 대명사·어미·감탄사로만 "
    "지칭되면(문장에 다른 사람 이름/호칭이 등장해도 마찬가지다, 그건 "
    "다른 사람 얘기다) — 1인칭 고백/감정 표현, 청자에게 하는 명령·평가 "
    "문장이 대표적인 함정이다. 고백·만취·눈물 같은 감정적 내용이나 "
    "통계적으로 흔한 성별상만으로 추정하지 말고 반드시 null로 남겨라 "
    "(사람에게 확인받는다 — 억지로 추측하지 마라). "
...
class GptClient:
    ...
    async def verify_and_refine(self, pairs: List[dict], profile: dict,
        ...
        system = (
            ...
            "     * 직역투로 인해 명백히 어색한 경우 (한국어 구조를 그대로 따라가 스페인어로서 부자연스러운 경우)\n"
