...
_JSON_INSTRUCTION_REQUERY = (
    "반드시 JSON 배열만 출력하라. 다른 설명 텍스트를 붙이지 마라. 이 세그먼트는 "
    "검수자가 이미 지적한 것이므로 배열에서 빼는 것은 금지된다 — 검토 도중 판단이 "
    ...
    async def correct_primary(self, pairs: List[dict], profile: dict,
        ...
        if extra_instruction:
            scope_intro = (
                ...
                "가장 가까운 카테고리를 찾아 검수자 지시사항을 반영한 교정 사항을 반드시 작성하라. "
        ...
        system = (
            ...
            "   - 기준: target_text에 이미 특정 성별 어미(형용사·분사·명사 어미)나 격식(존댓말/반말) 형태가 반영되어 있을 수 있다 — 그 형태가 사전상 어색하거나 비표준으로 보여도, 검수 과정에서 의도적으로 맞춘 것이니 임의로 '자연스럽게' 되돌리지 마라.\n"
