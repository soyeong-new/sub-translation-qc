...
from app.core.uploads import (
    save_upload, save_video_upload, UnsupportedFileType, SRT_EXTENSIONS,
)
...
async def upload_video(request: Request):
    ...
    인코딩해서 보낸다). 압축 없이 그대로 저장하고 곧장 경로를 반환한다
    (save_video_upload 참고 — 압축을 없애 별도 폴링이 필요 없어졌다)."""
    filename = request.headers.get("x-filename")
    if not filename:
        raise HTTPException(400, "X-Filename 헤더가 필요합니다")
    try:
        path = await save_video_upload(unquote(filename), request.stream())
